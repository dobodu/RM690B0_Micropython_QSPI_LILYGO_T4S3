...
#define RM690B0_DRIVER_VERSION "15.09.2024"
...
STATIC void write_spi(rm690b0_RM690B0_obj_t *self, int cmd, const void *buf, int len) {
    if (self->lcd_panel_p) {
...
STATIC void frame_buffer_alloc(rm690b0_RM690B0_obj_t *self, int len) {
...
STATIC void set_rotation(rm690b0_RM690B0_obj_t *self, uint8_t rotation) {
...
STATIC void rm690b0_RM690B0_print(const mp_print_t *print,
                                 mp_obj_t          self_in,
                                 mp_print_kind_t   kind)
{
    ...
    mp_printf(
        print,
        "<RM690B0 bus=%p, reset=%p, color_space=%s, bpp=%u>, version=%s",
        self->bus_obj,
        self->reset,
        color_space_desc[self->color_space],
        self->bpp,
        RM690B0_DRIVER_VERSION
    );
}


mp_obj_t rm690b0_RM690B0_make_new(const mp_obj_type_t *type,
                                 size_t               n_args,
                                 size_t               n_kw,
                                 const mp_obj_t      *all_args)
{
    ...
    // create new object
    rm690b0_RM690B0_obj_t *self = m_new_obj(rm690b0_RM690B0_obj_t);
    self->base.type = &rm690b0_RM690B0_type;

    self->bus_obj = (mp_obj_base_t *)MP_OBJ_TO_PTR(args[ARG_bus].u_obj);
#ifdef MP_OBJ_TYPE_GET_SLOT
    self->lcd_panel_p = (rm690b0_panel_p_t *)MP_OBJ_TYPE_GET_SLOT(self->bus_obj->type, protocol);
#else
    self->lcd_panel_p = (rm690b0_panel_p_t *)self->bus_obj->type->protocol;
#endif

    // self->max_width_value etc will be initialized in the rotation later.
    self->width = ((rm690b0_qspi_bus_obj_t *)self->bus_obj)->width;
    self->height = ((rm690b0_qspi_bus_obj_t *)self->bus_obj)->height;
...
{   
    return mp_obj_new_str(RM690B0_DRIVER_VERSION, 10);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_0(rm690b0_RM690B0_version_obj, rm690b0_RM690B0_version);


STATIC mp_obj_t rm690b0_RM690B0_deinit(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    //m_del_obj(rm690b0_RM690B0_obj_t, self); 
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_deinit_obj, rm690b0_RM690B0_deinit);


STATIC mp_obj_t rm690b0_RM690B0_reset(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_reset_obj, rm690b0_RM690B0_reset);



//Init function upgraded to match lilygo amoled github repo

STATIC mp_obj_t rm690b0_RM690B0_init(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_init_obj, rm690b0_RM690B0_init);


STATIC mp_obj_t rm690b0_RM690B0_send_cmd(size_t n_args, const mp_obj_t *args_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_send_cmd_obj, 4, 4, rm690b0_RM690B0_send_cmd);
...
STATIC mp_obj_t rm690b0_RM690B0_colorRGB(size_t n_args, const mp_obj_t *args_in) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_colorRGB_obj, 4, 4, rm690b0_RM690B0_colorRGB);


STATIC void set_area(rm690b0_RM690B0_obj_t *self, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
...
STATIC void refresh_display(rm690b0_RM690B0_obj_t *self, uint16_t x, uint16_t y, uint16_t w, uint16_t h){
	...
	for (uint16_t line = 0; line < h1; line++) {
		buf_idx = ((y0 + line) * self->width) + x0;
		for (uint16_t col = 0; col < w1; col++) {
			self->partial_frame_buffer[p_buf_idx] = self->frame_buffer[buf_idx];
			buf_idx++;	
			p_buf_idx++;
		}
	}
	...
	set_area(self, x0, y0, x1, y1);
	write_color(self, self->partial_frame_buffer, self->partial_frame_buffer_size);

	//Than free memory and return
	m_free(self->partial_frame_buffer);
}

STATIC mp_obj_t rm690b0_RM690B0_refresh(size_t n_args, const mp_obj_t *args_in) {
	rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
	refresh_display(self, 0, 0, self->width, self->height);  
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_refresh_obj, 1, 1, rm690b0_RM690B0_refresh);




// This fill the frame buffer area and 
STATIC void fill_frame_buffer(rm690b0_RM690B0_obj_t *self, uint16_t color, uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
	...
	if (w * h > self->frame_buffer_size / 2) {
        mp_raise_ValueError(MP_ERROR_TEXT("fill_frame_buffer: maximum length exceeded, please check dimensions."));
        return;
    }
    color = (color << 16) | color;
	
	// We update the frame_buffer
...
STATIC void pixel(rm690b0_RM690B0_obj_t *self, uint16_t x, uint16_t y, uint16_t color) {
...
STATIC mp_obj_t rm690b0_RM690B0_pixel(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_pixel_obj, 4, 4, rm690b0_RM690B0_pixel);


STATIC mp_obj_t rm690b0_RM690B0_fill(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_fill_obj, 2, 2, rm690b0_RM690B0_fill);


STATIC void fast_hline(rm690b0_RM690B0_obj_t *self, uint16_t x, uint16_t y, uint16_t len, uint16_t color) {
    if (len == 0) {
        return;
    }
    if (len == 1) {
        ...
        if (x + len > self->max_width_value) {
            len = self->max_width_value - x; // *1
        } 
        fill_frame_buffer(self, color, x, y, len, 1);
    }
}

STATIC mp_obj_t rm690b0_RM690B0_hline(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_hline_obj, 5, 5, rm690b0_RM690B0_hline);


STATIC void fast_vline(rm690b0_RM690B0_obj_t *self, uint16_t x, uint16_t y, uint16_t len, uint16_t color) {
    if (len == 0) {
        return;
    }
    if (len == 1) {
        ...
        if (y + len > self->max_height_value) {
            len = self->max_height_value - y; // *2
        }
        fill_frame_buffer(self, color, x, y, 1, len);
    }
}

STATIC mp_obj_t rm690b0_RM690B0_vline(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
    uint16_t len = mp_obj_get_int(args_in[3]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_vline_obj, 5, 5, rm690b0_RM690B0_vline);



STATIC void rect(rm690b0_RM690B0_obj_t *self, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
...
STATIC mp_obj_t rm690b0_RM690B0_rect(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_rect_obj, 6, 6, rm690b0_RM690B0_rect);
...
STATIC mp_obj_t rm690b0_RM690B0_fill_rect(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
    uint16_t w = mp_obj_get_int(args_in[3]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_fill_rect_obj, 6, 6, rm690b0_RM690B0_fill_rect);


STATIC void fill_bubble_rect(rm690b0_RM690B0_obj_t *self, uint16_t xs, uint16_t ys, uint16_t w, uint16_t h, uint16_t color) {
...
STATIC mp_obj_t rm690b0_RM690B0_fill_bubble_rect(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
    uint16_t w = mp_obj_get_int(args_in[3]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_fill_bubble_rect_obj, 6, 6, rm690b0_RM690B0_fill_bubble_rect);


STATIC void bubble_rect(rm690b0_RM690B0_obj_t *self, uint16_t xs, uint16_t ys, uint16_t w, uint16_t h, uint16_t color) {
    if (xs + w > self->width || ys + h > self->height) {
        return;
    }
    int bubble_size;
...
STATIC mp_obj_t rm690b0_RM690B0_bubble_rect(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t x = mp_obj_get_int(args_in[1]);
    uint16_t y = mp_obj_get_int(args_in[2]);
    uint16_t w = mp_obj_get_int(args_in[3]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_bubble_rect_obj, 6, 6, rm690b0_RM690B0_bubble_rect);


STATIC void circle(rm690b0_RM690B0_obj_t *self, uint16_t xm, uint16_t ym, uint16_t r, uint16_t color) {
    int x = 0;
    int y = r;
    int p = 1 - r;
	...
	self->hold_display = false;
	refresh_display(self,xm-r,ym-r,2*r,2*r);
}

STATIC mp_obj_t rm690b0_RM690B0_circle(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_circle_obj, 5, 5, rm690b0_RM690B0_circle);


STATIC void fill_circle(rm690b0_RM690B0_obj_t *self, uint16_t xm, uint16_t ym, uint16_t r, uint16_t color) {
    int x = 0;
    int y = r;
    int p = 1 - r;
...
STATIC mp_obj_t rm690b0_RM690B0_fill_circle(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
    uint16_t xm = mp_obj_get_int(args_in[1]);
    uint16_t ym = mp_obj_get_int(args_in[2]);
    uint16_t r = mp_obj_get_int(args_in[3]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_fill_circle_obj, 5, 5, rm690b0_RM690B0_fill_circle);


STATIC void line(rm690b0_RM690B0_obj_t *self, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
	...
	self->hold_display = false;
	if (y1 > y0) {
		refresh_display(self,x0,y0,x1-x0,y1-y0);
	} else {
		refresh_display(self,x0,y1,x1-x0,y0-y1);
	}
}


STATIC mp_obj_t rm690b0_RM690B0_line(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_line_obj, 6, 6, rm690b0_RM690B0_line);


// Return the center of a polygon as an (x, y) tuple
STATIC mp_obj_t rm690b0_RM690B0_polygon_center(size_t n_args, const mp_obj_t *args) {
    size_t poly_len;
    mp_obj_t *polygon;
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_polygon_center_obj, 2, 2, rm690b0_RM690B0_polygon_center);
...
STATIC void PolygonFill(rm690b0_RM690B0_obj_t *self, Polygon *polygon, Point location, uint16_t color) {
...
STATIC mp_obj_t rm690b0_RM690B0_polygon(size_t n_args, const mp_obj_t *args) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_polygon_obj, 4, 8, rm690b0_RM690B0_polygon);


STATIC mp_obj_t rm690b0_RM690B0_fill_polygon(size_t n_args, const mp_obj_t *args) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_fill_polygon_obj, 4, 8, rm690b0_RM690B0_fill_polygon);


STATIC mp_obj_t rm690b0_RM690B0_bitmap(size_t n_args, const mp_obj_t *args_in) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_bitmap_obj, 6, 6, rm690b0_RM690B0_bitmap);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_text_obj, 5, 7, rm690b0_RM690B0_text);
...
STATIC mp_obj_t rm690b0_RM690B0_text_len(size_t n_args, const mp_obj_t *args) {
    //rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args[0]);
    uint8_t single_char_s;
    const uint8_t *source = NULL;
    size_t source_len = 0;
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_text_len_obj, 3, 3, rm690b0_RM690B0_text_len);


STATIC mp_obj_t rm690b0_RM690B0_write_len(size_t n_args, const mp_obj_t *args) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_write_len_obj, 3, 3, rm690b0_RM690B0_write_len);
...
typedef struct {
    ...
    rm690b0_RM690B0_obj_t *self;  // display object
...
static mp_obj_t rm690b0_RM690B0_jpg(size_t n_args, const mp_obj_t *args) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args[0]);
			...
			if (res == JDR_OK) {
				...
				for(uint16_t line=0; line < jdec.height; line++) {
					...
					for(uint16_t col=0; col < jdec.width; col++) {
						...
						self->frame_buffer[buf_idx] = color;
						//self->frame_buffer[buf_idx] = devid.fbuf[jpg_idx];
						buf_idx++;
						//jpg_idx++;
						jpg_idx += 2;
					}
...
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_jpg_obj, 4, 5, rm690b0_RM690B0_jpg);
...
static mp_obj_t rm690b0_RM690B0_jpg_decode(size_t n_args, const mp_obj_t *args) {
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args[0]);
	const char	*filename;
...
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_jpg_decode_obj, 2, 6, rm690b0_RM690B0_jpg_decode);
...
STATIC mp_obj_t rm690b0_RM690B0_mirror(mp_obj_t self_in,
...
STATIC MP_DEFINE_CONST_FUN_OBJ_3(rm690b0_RM690B0_mirror_obj, rm690b0_RM690B0_mirror);


STATIC mp_obj_t rm690b0_RM690B0_swap_xy(mp_obj_t self_in, mp_obj_t swap_axes_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(rm690b0_RM690B0_swap_xy_obj, rm690b0_RM690B0_swap_xy);


STATIC mp_obj_t rm690b0_RM690B0_set_gap(mp_obj_t self_in,
                                       mp_obj_t x_gap_in,
                                       mp_obj_t y_gap_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_3(rm690b0_RM690B0_set_gap_obj, rm690b0_RM690B0_set_gap);


STATIC mp_obj_t rm690b0_RM690B0_invert_color(mp_obj_t self_in, mp_obj_t invert_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(rm690b0_RM690B0_invert_color_obj, rm690b0_RM690B0_invert_color);


STATIC mp_obj_t rm690b0_RM690B0_disp_off(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_disp_off_obj, rm690b0_RM690B0_disp_off);


STATIC mp_obj_t rm690b0_RM690B0_disp_on(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_disp_on_obj, rm690b0_RM690B0_disp_on);


STATIC mp_obj_t rm690b0_RM690B0_backlight_on(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_backlight_on_obj, rm690b0_RM690B0_backlight_on);


STATIC mp_obj_t rm690b0_RM690B0_backlight_off(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_backlight_off_obj, rm690b0_RM690B0_backlight_off);


STATIC mp_obj_t rm690b0_RM690B0_brightness(mp_obj_t self_in, mp_obj_t brightness_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(rm690b0_RM690B0_brightness_obj, rm690b0_RM690B0_brightness);


STATIC mp_obj_t rm690b0_RM690B0_width(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
    return mp_obj_new_int(self->width);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_width_obj, rm690b0_RM690B0_width);


STATIC mp_obj_t rm690b0_RM690B0_height(mp_obj_t self_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(self_in);
    return mp_obj_new_int(self->height);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_RM690B0_height_obj, rm690b0_RM690B0_height);


STATIC mp_obj_t rm690b0_RM690B0_rotation(size_t n_args, const mp_obj_t *args_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_rotation_obj, 2, 3, rm690b0_RM690B0_rotation);


STATIC mp_obj_t rm690b0_RM690B0_vscroll_area(size_t n_args, const mp_obj_t *args_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_vscroll_area_obj, 4, 4, rm690b0_RM690B0_vscroll_area);


STATIC mp_obj_t rm690b0_RM690B0_vscroll_start(size_t n_args, const mp_obj_t *args_in)
{
    rm690b0_RM690B0_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_RM690B0_vscroll_start_obj, 2, 3, rm690b0_RM690B0_vscroll_start);


// Mapping to Micropython
STATIC const mp_rom_map_elem_t rm690b0_RM690B0_locals_dict_table[] = {
    /* { MP_ROM_QSTR(MP_QSTR_custom_init),   MP_ROM_PTR(&rm690b0_RM690B0_custom_init_obj)   }, */
    { MP_ROM_QSTR(MP_QSTR_version),         MP_ROM_PTR(&rm690b0_RM690B0_version_obj)          },
    { MP_ROM_QSTR(MP_QSTR_deinit),          MP_ROM_PTR(&rm690b0_RM690B0_deinit_obj)          },
    { MP_ROM_QSTR(MP_QSTR_reset),           MP_ROM_PTR(&rm690b0_RM690B0_reset_obj)           },
    { MP_ROM_QSTR(MP_QSTR_init),            MP_ROM_PTR(&rm690b0_RM690B0_init_obj)            },
    { MP_ROM_QSTR(MP_QSTR_send_cmd),        MP_ROM_PTR(&rm690b0_RM690B0_send_cmd_obj)        },
	{ MP_ROM_QSTR(MP_QSTR_refresh),         MP_ROM_PTR(&rm690b0_RM690B0_refresh_obj)         },
    { MP_ROM_QSTR(MP_QSTR_pixel),           MP_ROM_PTR(&rm690b0_RM690B0_pixel_obj)           },
    { MP_ROM_QSTR(MP_QSTR_write_len),       MP_ROM_PTR(&rm690b0_RM690B0_write_len_obj)       },
    { MP_ROM_QSTR(MP_QSTR_write),           MP_ROM_PTR(&rm690b0_RM690B0_write_obj)           },
    ...
    { MP_ROM_QSTR(MP_QSTR_fill),            MP_ROM_PTR(&rm690b0_RM690B0_fill_obj)            },
    { MP_ROM_QSTR(MP_QSTR_fill_rect),       MP_ROM_PTR(&rm690b0_RM690B0_fill_rect_obj)       },
    { MP_ROM_QSTR(MP_QSTR_fill_bubble_rect),MP_ROM_PTR(&rm690b0_RM690B0_fill_bubble_rect_obj)},
    ...
    { MP_ROM_QSTR(MP_QSTR_fill_polygon),    MP_ROM_PTR(&rm690b0_RM690B0_fill_polygon_obj)    },
    { MP_ROM_QSTR(MP_QSTR_polygon),         MP_ROM_PTR(&rm690b0_RM690B0_polygon_obj)         },
    { MP_ROM_QSTR(MP_QSTR_polygon_center),  MP_ROM_PTR(&rm690b0_RM690B0_polygon_center_obj)  },
    { MP_ROM_QSTR(MP_QSTR_rect),            MP_ROM_PTR(&rm690b0_RM690B0_rect_obj)            },
    { MP_ROM_QSTR(MP_QSTR_bubble_rect),     MP_ROM_PTR(&rm690b0_RM690B0_bubble_rect_obj)     },
    { MP_ROM_QSTR(MP_QSTR_circle),          MP_ROM_PTR(&rm690b0_RM690B0_circle_obj)          },
    { MP_ROM_QSTR(MP_QSTR_colorRGB),        MP_ROM_PTR(&rm690b0_RM690B0_colorRGB_obj)        },
    { MP_ROM_QSTR(MP_QSTR_bitmap),          MP_ROM_PTR(&rm690b0_RM690B0_bitmap_obj)          },
	{ MP_ROM_QSTR(MP_QSTR_jpg),             MP_ROM_PTR(&rm690b0_RM690B0_jpg_obj)             },
    { MP_ROM_QSTR(MP_QSTR_jpg_decode),      MP_ROM_PTR(&rm690b0_RM690B0_jpg_decode_obj)      },
    { MP_ROM_QSTR(MP_QSTR_text),            MP_ROM_PTR(&rm690b0_RM690B0_text_obj)            },
	{MP_ROM_QSTR(MP_QSTR_text_len),         MP_ROM_PTR(&rm690b0_RM690B0_text_len_obj)        },
    { MP_ROM_QSTR(MP_QSTR_mirror),          MP_ROM_PTR(&rm690b0_RM690B0_mirror_obj)          },
    { MP_ROM_QSTR(MP_QSTR_swap_xy),         MP_ROM_PTR(&rm690b0_RM690B0_swap_xy_obj)         },
    { MP_ROM_QSTR(MP_QSTR_set_gap),         MP_ROM_PTR(&rm690b0_RM690B0_set_gap_obj)         },
    { MP_ROM_QSTR(MP_QSTR_invert_color),    MP_ROM_PTR(&rm690b0_RM690B0_invert_color_obj)    },
    { MP_ROM_QSTR(MP_QSTR_disp_off),        MP_ROM_PTR(&rm690b0_RM690B0_disp_off_obj)        },
    { MP_ROM_QSTR(MP_QSTR_disp_on),         MP_ROM_PTR(&rm690b0_RM690B0_disp_on_obj)         },
    { MP_ROM_QSTR(MP_QSTR_backlight_on),    MP_ROM_PTR(&rm690b0_RM690B0_backlight_on_obj)    },
    { MP_ROM_QSTR(MP_QSTR_backlight_off),   MP_ROM_PTR(&rm690b0_RM690B0_backlight_off_obj)   },
    ...
    { MP_ROM_QSTR(MP_QSTR_vscroll_area),    MP_ROM_PTR(&rm690b0_RM690B0_vscroll_area_obj)    },
    { MP_ROM_QSTR(MP_QSTR_vscroll_start),   MP_ROM_PTR(&rm690b0_RM690B0_vscroll_start_obj)   },
    { MP_ROM_QSTR(MP_QSTR___del__),         MP_ROM_PTR(&rm690b0_RM690B0_deinit_obj)          },
...
MP_DEFINE_CONST_OBJ_TYPE(
    ...
    MP_QSTR_RM690B0,
    MP_TYPE_FLAG_NONE,
    print, rm690b0_RM690B0_print,
    make_new, rm690b0_RM690B0_make_new,
    locals_dict, (mp_obj_dict_t *)&rm690b0_RM690B0_locals_dict
);
#else
const mp_obj_type_t rm690b0_RM690B0_type = {
    { &mp_type_type },
    .name        = MP_QSTR_RM690B0,
    .print       = rm690b0_RM690B0_print,
    .make_new    = rm690b0_RM690B0_make_new,
    .locals_dict = (mp_obj_dict_t *)&rm690b0_RM690B0_locals_dict,
};
...
STATIC MP_DEFINE_CONST_DICT(mp_module_rm690b0_globals, mp_module_rm690b0_globals_table);


const mp_obj_module_t mp_module_rm690b0 = {
    .base    = {&mp_type_module},
    .globals = (mp_obj_dict_t *)&mp_module_rm690b0_globals,
};


#if MICROPY_VERSION >= 0x011300 // MicroPython 1.19 or later
MP_REGISTER_MODULE(MP_QSTR_rm690b0, mp_module_rm690b0);
#else
MP_REGISTER_MODULE(MP_QSTR_rm690b0, mp_module_rm690b0, MODULE_RM690B0_ENABLE);
#endif
