...
STATIC void rm690b0_qspi_bus_print(const mp_print_t *print,
                                    mp_obj_t          self_in,
                                    mp_print_kind_t   kind)
{
    (void) kind;
    rm690b0_qspi_bus_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_printf(
        print,
...
STATIC mp_obj_t rm690b0_qspi_bus_make_new(const mp_obj_type_t *type,
                                           size_t               n_args,
                                           size_t               n_kw,
                                           const mp_obj_t      *all_args)
{
    ...
    // create new object
    rm690b0_qspi_bus_obj_t *self = m_new_obj(rm690b0_qspi_bus_obj_t);
    self->base.type = &rm690b0_qspi_bus_type;
...
STATIC mp_obj_t rm690b0_qspi_bus_tx_param(size_t n_args, const mp_obj_t *args_in)
{
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_qspi_bus_tx_param_obj, 2, 3, rm690b0_qspi_bus_tx_param);


STATIC mp_obj_t rm690b0_qspi_bus_tx_color(size_t n_args, const mp_obj_t *args_in)
{
    mp_obj_base_t *self = (mp_obj_base_t *)MP_OBJ_TO_PTR(args_in[0]);
    int cmd = mp_obj_get_int(args_in[1]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(rm690b0_qspi_bus_tx_color_obj, 2, 3, rm690b0_qspi_bus_tx_color);


STATIC mp_obj_t rm690b0_qspi_bus_deinit(mp_obj_t self_in)
{
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(rm690b0_qspi_bus_deinit_obj, rm690b0_qspi_bus_deinit);


STATIC const mp_rom_map_elem_t rm690b0_qspi_bus_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_tx_param), MP_ROM_PTR(&rm690b0_qspi_bus_tx_param_obj) },
    { MP_ROM_QSTR(MP_QSTR_tx_color), MP_ROM_PTR(&rm690b0_qspi_bus_tx_color_obj) },
    { MP_ROM_QSTR(MP_QSTR_deinit),   MP_ROM_PTR(&rm690b0_qspi_bus_deinit_obj)   },
    { MP_ROM_QSTR(MP_QSTR___del__),  MP_ROM_PTR(&rm690b0_qspi_bus_deinit_obj)   },
};
STATIC MP_DEFINE_CONST_DICT(rm690b0_qspi_bus_locals_dict, rm690b0_qspi_bus_locals_dict_table);


STATIC const rm690b0_panel_p_t mp_lcd_panel_p = {
...
MP_DEFINE_CONST_OBJ_TYPE(
    rm690b0_qspi_bus_type,
    MP_QSTR_QSPI_Panel,
    MP_TYPE_FLAG_NONE,
    print, rm690b0_qspi_bus_print,
    make_new, rm690b0_qspi_bus_make_new,
    protocol, &mp_lcd_panel_p,
    locals_dict, (mp_obj_dict_t *)&rm690b0_qspi_bus_locals_dict
);
#else
const mp_obj_type_t rm690b0_qspi_bus_type = {
    { &mp_type_type },
